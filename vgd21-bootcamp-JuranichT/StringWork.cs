...
namespace vgd21_bootcamp_JuranichT
{
    public static class StringWork
    {
        ...
        public static void HomeWorkChallenge()
        {
            ...
            while (nameLength > 11)
            {
                Console.WriteLine("Enter a name that has less then 10 charechters in it. > ");
            ...
            string lower = userEntry.ToLower();
            Console.WriteLine("{0}", lower);


            //Q3
            Console.WriteLine("Make some weapons seperating each with a comma. > ");
            string Weapons = Console.ReadLine();
            string phrase = Weapons;
            string[] words = phrase.Split(',');

            Console.WriteLine("You listed {0} weapons.", words.Length);
            for (int i = 0; i < words.Length; ++i)
            {
                Console.WriteLine("{0}: {1}", i + 1, words[i].Trim());
            }
            ...
            DateTime birthday = new DateTime(Y + 10, M, D);
